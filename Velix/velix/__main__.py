"""
Velix — Ponto de Entrada Centralizado

Uso:
    python -m velix              # Modo produção (normal)
    python -m velix --dev        # Modo desenvolvimento
    python -m velix --test       # Rodar testes
    python -m velix --build      # Gerar executável .exe
    python -m velix --installer  # Gerar instalador
    python -m velix --help       # Ver todas as opções
"""

import sys
from pathlib import Path

# Add root directory to Python path (for ai module imports)
app_root = Path(__file__).parent.parent
sys.path.insert(0, str(app_root))

import argparse
from velix.config import AppConfig
from velix.cli import CLIManager
from velix.version import __version__


def create_parser() -> argparse.ArgumentParser:
    """Cria parser de argumentos da CLI."""
    parser = argparse.ArgumentParser(
        description="Velix — Software para Personal Trainers Profissionais",
        prog="python -m velix",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Exemplos:
  python -m velix              # Rodar aplicação (modo produção)
  python -m velix --dev        # Rodar em desenvolvimento
  python -m velix --test       # Rodar testes
  python -m velix --build      # Gerar executável
  python -m velix --version    # Mostrar versão
