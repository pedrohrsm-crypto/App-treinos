...
from flet_app.theme import c, PHASE_COLORS
...
def calendar_view_screen(page: ft.Page, route: str) -> ft.View:
    ...
    def _update_month_label():
        ...
        month_label.value = f"{month_names[view_month[0]]} {view_year[0]}"
    ...
    month_nav = ft.Row(
        [
            ...
            ft.TextButton("Hoje", on_click=_go_today),
        ],
    ...
    def _on_drag_accept(e):
        ...
        if not drag_data or not target_date or drag_data["date"] == target_date:
            return

        ok = training_manager.move_workout(
            trainer, plan_id,
            from_date=drag_data["date"],
            session_index=drag_data["session_index"],
            to_date=target_date,
        )
        if ok:
            undo_state["from"] = drag_data["date"]
            undo_state["to"] = target_date
            undo_state["index"] = drag_data["session_index"]
            _refresh_calendar()
    ...
    def _build_day_cell(d: date, is_current_month: bool) -> ft.Container:
        ...
        for i, s in enumerate(sessions[:3]):
            ...
            emoji = SPORT_EMOJI_SHORT.get(modalidade, "📋")
            tipo_short = s.get("tipo", "")[:3]

            chip_content = ft.Container(
                content=ft.Text(f"{emoji}{tipo_short}", size=9, color="#FFF"),
                bgcolor=zone_color,
                border_radius=4,
            ...
            if is_current_month:
                ...
                draggable = ft.Draggable(
                    ...
                    content_feedback=ft.Container(
                        content=ft.Text(f"{emoji}{tipo_short}", size=11, color="#FFF"),
                        bgcolor=zone_color,
                        border_radius=6,
        ...
        if len(sessions) > 3:
            chips.append(ft.Text(f"+{len(sessions) - 3}", size=9, color=c("text_secondary", dark)))
    ...
    def _open_day_detail(date_key: str, sessions: list):
            ...
            card = ft.Container(
                content=ft.Column([
                    ft.Row([
                        ...
                        ft.Column([
                            ft.Text(f"{emoji} {s.get('tipo', '')} — {s.get('duracao', '')}", size=14, weight=ft.FontWeight.W_600),
        ...
        sheet = ft.BottomSheet(
            content=ft.Container(
                content=ft.Column(
                    [
                        ft.Text(f"📅 {date_key}", size=16, weight=ft.FontWeight.BOLD),
                        ft.Text(f"{len(full_sessions)} sessão(ões)", size=13, color=c("text_secondary", dark)),
                        ft.Divider(),
                        *session_cards,
                    ],
                    spacing=8,
                ...
                content=ft.Column(
                    [
                        ft.Row([
                            ...
                            ft.Text("Calendário de Treino", size=18, weight=ft.FontWeight.BOLD, expand=True),
                        ]),
                        month_nav,
                        grid_column,
                        ft.Divider(),
