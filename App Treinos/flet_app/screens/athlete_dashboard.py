...
def athlete_dashboard_view(page: ft.Page, route: str) -> ft.View:
    ...
    # Calcular semana atual para cada plano
    now = datetime.now()
    ...
    def _delete_plan(e):
        ...
        if ok:
            page.go(f"/athlete/{athlete_name}")  # reload
        else:
    ...
    for p in athlete_plans:
        ...
        card = build_plan_card(
            p, current_week=cw,
            on_calendar=_go_calendar,
            on_delete=_delete_plan,
            on_export=_export_plan,
            dark=dark,
        )
    ...
    if not plan_cards:
        plan_cards.append(
            ft.Container(
                ft.Text("Nenhum plano criado para este atleta.", color=c("text_secondary", dark)),
                padding=20,
            )
        )
    ...
    content = ft.Column(
        [header, stats_row, ft.Divider(height=16, color=ft.Colors.TRANSPARENT),
    ...
    return ft.View(
        ...
        controls=[ft.Container(content=content, padding=ft.padding.symmetric(horizontal=8), expand=True)],
